...
# % GramSchmidt process is numerically unstable. If we write an algorithm
# % based on the way we developed the Gram-Schmidt iteration (in terms of 
# % projections), we get a better algorithm.
# % The GramSchmidt process can be stabilized by a small modification. 
...
function mgrscho!(A::AbstractMatrix)
...
mgrscho(A::AbstractMatrix) = mgrscho!(copy(A))